...
{
    $userid = $_GET["id"];
    $sql = "SELECT * FROM reservation WHERE id = :userid";
...
elseif (isset($_POST["id"]) && isset($_POST["name"]) && isset($_POST["date"])) {
    ...
    $sql = "UPDATE reservation SET date = :userdate, time = :usertime, guestcount = :userguestcount, name = :username, phonenumber = :userphonenumber, email = :useremail WHERE id = :userid";
    $stmt = $conn->prepare($sql);
